...
parser.add_argument('--dataPath', type=str, default='/media/second/data/netvlad/', help='Path for centroid data.')
parser.add_argument('--runsPath', type=str, default='/media/second/runs/netvlad/', help='Path to save runs to.')
parser.add_argument('--savePath', type=str, default='checkpoints', 
        help='Path to save checkpoints to in logdir. Default=checkpoints/')
parser.add_argument('--cachePath', type=str, default=tempfile.mkdtemp(), help='Path to save cache to.')
...
parser.add_argument('--pooling', type=str, default='netvlad', help='type of pooling to use',
        choices=['netvlad', 'max', 'avg', 'graph'])
parser.add_argument('--num_clusters', type=int, default=64, help='Number of NetVlad clusters. Default=64')
parser.add_argument('--num_graph_layers', type=int, default=2, help='number of attentional + message passing layers')
...
def train(epoch):
    ...
    for subIter in range(subsetN):
        ...
        with h5py.File(train_set.cache, mode='w') as h5: 
            ...
            if opt.pooling.lower() == 'graph': pool_size *= 2
            h5feat = h5.create_dataset("features", 
                    [len(whole_train_set), pool_size], 
                    dtype=np.float32)
            with torch.no_grad():
                for iteration, (input, indices) in enumerate(tqdm(whole_training_data_loader), 1):
        ...
        for iteration, (query, positives, negatives, 
            ...
            # print("before", torch.cuda.memory_allocated(), torch.cuda.memory_cached())
            input = input.to(device)
            # print("image", torch.cuda.memory_allocated(), torch.cuda.memory_cached())
            image_encoding = model.encoder(input)
            # print("encode", torch.cuda.memory_allocated(), torch.cuda.memory_cached())
            # image_encoding = image_encoding.detach()
            # input = input.detach()
            # print("detach", torch.cuda.memory_allocated(), torch.cuda.memory_cached())
            # print(len(query), len(positives), len(negatives), image_encoding.shape)

            vlad_encoding = model.pool(image_encoding) 
            # print("pooled", torch.cuda.memory_allocated(), torch.cuda.memory_cached())

            vladQ, vladP, vladN = torch.split(vlad_encoding, [B, B, nNeg])
            # print("split", torch.cuda.memory_allocated(), torch.cuda.memory_cached())
...
if __name__ == "__main__":
    ...
    if opt.mode.lower() != 'cluster':
        if opt.pooling.lower() == 'netvlad':
            ...
            model.add_module('pool', nn.Sequential(*[global_pool, Flatten(), L2Norm()]))
        elif opt.pooling.lower() == 'graph':
            coord_layers = []
            hidden = 32
            while hidden < encoder_dim:
                coord_layers.append(hidden)
                hidden *= 2

            graph_config = {'descriptor_dim': encoder_dim,
                            'coord_encoder': coord_layers,
                            'GNN_layers': opt.num_graph_layers,
                            'agg_hidden': encoder_dim * 2,
                            }
            graph_mod = graph.GraphLayer(graph_config)
            model.add_module('pool', graph_mod)
        else:
